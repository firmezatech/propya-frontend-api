...
const formatDisplayValue = (value: string | null | undefined, isCurrency: boolean = false) => {
  // Debug: Verificar o valor recebido
  console.log("🔍 formatDisplayValue recebeu:", value, "tipo:", typeof value, "isCurrency:", isCurrency);
  ...
  // Se for para formatar como moeda, converter o valor
  if (isCurrency) {
    // Para valores brasileiros, remover pontos (separadores de milhares) e substituir vírgula por ponto
    let cleanValue = value.toString();
    
    // Se contém vírgula, é formato brasileiro (ex: 1.000,50)
    if (cleanValue.includes(',')) {
      // Remove pontos (separadores de milhares) e substitui vírgula por ponto
      cleanValue = cleanValue.replace(/\./g, '').replace(',', '.');
    } else if (cleanValue.includes('.')) {
      // Se só tem ponto, pode ser separador de milhares (ex: 1.000) ou decimal (ex: 1.5)
      // Se tem mais de 3 dígitos após o último ponto, é separador de milhares
    ...
    if (!isNaN(numericValue)) {
      const formattedValue = formatCurrency.format(numericValue);
      console.log("🔍 Valor original:", value, "Valor limpo:", cleanValue, "Valor formatado:", formattedValue);
                ...
                <thead className="bg-gray-50 px-6">
                  <tr className="border-b border-gray-300">
                    ...
                    <th className="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                      <div className="inline-block">
                        <div className="text-left">{t('transactionValueLegacy.line1')}</div>
                        <div className="flex items-center justify-end">
                          {t('transactionValueLegacy.line2')}&nbsp;
                          <InfoTooltip content={t('transactionValueInfoLegacy')} />
                        </div>
                      </div>
                    </th>
                    <th className="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                      <div className="inline-block">
                        <div className="text-left">{t('remainingTokensLegacy.line1')}</div>
                        <div className="flex items-center justify-end">
                          {t('remainingTokensLegacy.line2')}&nbsp;
                          <InfoTooltip content={t('remainingTokensInfoLegacy')} />
                        </div>
                      </div>
                    </th>
                  ...
                  {displayedItems.map((item, index) => (
                    <React.Fragment key={index}>
                      <tr className="hover:bg-gray-50">
                        ...
                        <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-500 text-right">{formatDisplayValue(item.tokensRemaining, true)}</td>
                      </tr>
                    </React.Fragment>
                  ))}
                </tbody>
