...
export const fmzAdminShellLayoutConfig = {
  root: 'flex h-[calc(100dvh-72px)] min-h-0 flex-1 overflow-hidden bg-[#F7F8FA]',
  contentScroll: 'min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden bg-[#F7F8FA]',
} as const;

export const fmzAdminSidebarLayoutConfig = {
  sidebar: 'z-30 hidden h-full min-h-0 w-[clamp(280px,18vw,320px)] shrink-0 flex-col border-r border-fmz-border-light bg-white lg:flex',
  navigationArea: 'flex min-h-0 min-w-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden px-5 py-5',
  link: 'flex min-w-0 w-full items-start gap-2.5 rounded-xl px-3 py-2.5 text-[13px] no-underline transition',
  ...
  desktopFooter: 'hidden shrink-0 border-t border-fmz-border-light p-5 lg:block',
  mobileFooter: 'hidden',
