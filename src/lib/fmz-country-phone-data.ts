// Centralized country / dialing-code dataset for the phone country selector.
// Source of truth: ISO 3166-1 alpha-2 codes + E.164 country calling codes.
// The flag is derived from the ISO2 code (regional-indicator emoji) so no image
// assets are needed and the list stays a plain, auditable data table.

export type FmzCountryPhone = {
  iso2: string;
  name: string;
  dialCode: string;
  flag: string;
...
// Countries surfaced at the top of the list (the app is Brazil-first). The rest
// follow alphabetically by name.
const PRIORITY_ISO2 = ['BR', 'PT', 'US'] as const;
...
const ISO2_TO_REGIONAL_INDICATOR_OFFSET = 0x1f1e6 - 'A'.charCodeAt(0);

/** Converts an ISO 3166-1 alpha-2 code into its flag emoji (🇧🇷 for "BR"). */
export const getFmzCountryFlag = (iso2: string): string =>
  iso2
    .toUpperCase()
    .replace(/[^A-Z]/g, '')
    .split('')
    .map((char) => String.fromCodePoint(char.charCodeAt(0) + ISO2_TO_REGIONAL_INDICATOR_OFFSET))
    .join('');

const toCountryPhone = ([iso2, name, dialCode]: readonly [string, string, string]): FmzCountryPhone => ({
  iso2,
  name,
  dialCode,
  flag: getFmzCountryFlag(iso2),
});

const priorityRank = (iso2: string): number => {
  const index = PRIORITY_ISO2.indexOf(iso2 as (typeof PRIORITY_ISO2)[number]);
  return index === -1 ? PRIORITY_ISO2.length : index;
};

// Priority countries first (in PRIORITY_ISO2 order), then the rest alphabetically.
export const FMZ_COUNTRY_PHONES: ReadonlyArray<FmzCountryPhone> = COUNTRY_TABLE.map(toCountryPhone).sort(
  (a, b) => priorityRank(a.iso2) - priorityRank(b.iso2) || a.name.localeCompare(b.name),
);

const FMZ_COUNTRY_PHONE_BY_ISO2 = new Map(FMZ_COUNTRY_PHONES.map((country) => [country.iso2, country]));

export const FMZ_DEFAULT_COUNTRY_ISO2 = 'BR';

/**
 * Resolves a stored value to a country. Accepts an ISO2 code ("BR") or a dial
 * code ("+55") for resilience against legacy data, falling back to Brazil.
 */
export const resolveFmzCountryPhone = (value: string | null | undefined): FmzCountryPhone => {
  const normalized = (value ?? '').trim();

  if (normalized) {
    const byIso2 = FMZ_COUNTRY_PHONE_BY_ISO2.get(normalized.toUpperCase());
    if (byIso2) return byIso2;

    const byDialCode = FMZ_COUNTRY_PHONES.find((country) => country.dialCode === normalized);
    if (byDialCode) return byDialCode;
  }

  return FMZ_COUNTRY_PHONE_BY_ISO2.get(FMZ_DEFAULT_COUNTRY_ISO2) ?? FMZ_COUNTRY_PHONES[0];
};
