  ...
  return (
    <div className="mb-6 rounded-2xl border border-[#E8EAF0] bg-white p-4 shadow-sm sm:flex sm:items-center sm:justify-between sm:p-5">
      <div>
        ...
        <h1 className="mt-1 font-sans text-xl font-extrabold tracking-[-.02em] text-[#0D1321]">{isEditing ? `Editar ${userName || 'usuário'}` : 'Novo usuário'}</h1>
        <p className="mt-1 text-[12.5px] text-[#5A6478]">Campos de wallet, contratos e percentuais são somente leitura e vêm do backend.</p>
      </div>
      <button onClick={onBack} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-[9px] border border-[#E8EAF0] bg-[#F7F8FA] px-4 py-3 font-sans text-[12px] font-bold uppercase tracking-[.04em] text-[#0D1321] transition hover:-translate-y-0.5 hover:border-[#0D1321] sm:mt-0 sm:w-auto">
      ...
      <div className="shrink-0">
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="font-sans text-[clamp(24px,4vw,34px)] font-extrabold tracking-[-.025em]">Usuários</h1>
            <p className="mt-1 max-w-2xl text-[13px] leading-6 text-[#5A6478]">Cadastre, edite e defina quais acessos cada usuário possui.</p>
          </div>
          <button onClick={onCreate} className="inline-flex w-full items-center justify-center gap-2 rounded-[9px] bg-[#0D1321] px-6 py-3 font-sans text-[13px] font-bold uppercase tracking-[.04em] text-white transition hover:-translate-y-0.5 hover:bg-[#162030] sm:w-auto">
...
function OwnershipView({ item }: { item: FmzAdminCoOwnerProperty }) {
  return <div className="rounded-xl border border-[#E8EAF0] bg-[#F7F8FA] p-3"><div className="flex items-start justify-between gap-3"><div><p className="font-sans text-[13.5px] font-bold text-[#0D1321]">{item.property?.name || 'Propriedade'}</p><p className="mt-1 text-[11.5px] leading-5 text-[#5A6478]">{propertyAddress(item.property)}</p></div><span className="rounded-full border border-[#A8DFC4] bg-[#F0FAF5] px-2.5 py-1 text-[10.5px] font-bold text-[#1A8C5B]">{formatPercentage(item.ownershipPercentage)}</span></div><div className="mt-3 grid gap-2 text-[11.5px] text-[#5A6478] sm:grid-cols-3"><Metric label="Tokens" value={`${formatTokenAmount(item.tokenBalance)} ${item.tokenization?.tokenSymbol ?? ''}`.trim()} /><Metric label="Pendente" value={formatTokenAmount(item.pendingTokenBalance)} /><Metric label="Token ID" value={String(item.tokenization?.tokenId ?? '—')} /></div></div>;
}

function RelationView({ relation }: { relation: FmzAdminPropertyRelation }) {
  return <div className="rounded-xl border border-[#E8EAF0] bg-white p-3"><div className="flex items-start justify-between gap-3"><div><p className="font-sans text-[13.5px] font-bold text-[#0D1321]">{relation.property?.name || 'Propriedade'}</p><p className="mt-1 text-[11.5px] leading-5 text-[#5A6478]">{propertyAddress(relation.property)}</p></div><Badge>{relation.relationType}</Badge></div></div>;
}

function ContractsCard({ contracts }: { contracts: FmzAdminTenantContract[] }) {
  return <DetailCard icon={<Home className="h-4 w-4" />} title="Contratos de inquilino" subtitle="Contratos ativos ou históricos retornados pelo backend"><div className="flex flex-col gap-3">{contracts.length ? contracts.map((contract) => <div key={contract.id} className="rounded-xl border border-[#E8EAF0] bg-[#F7F8FA] p-3"><div className="flex items-start justify-between gap-3"><div><p className="font-sans text-[13.5px] font-bold text-[#0D1321]">{contract.property?.name || contract.contractNumber || 'Contrato'}</p><p className="mt-1 text-[11.5px] text-[#5A6478]">{propertyAddress(contract.property)}</p></div><Badge>{contract.status || 'sem status'}</Badge></div><div className="mt-3 grid gap-2 text-[11.5px] text-[#5A6478] sm:grid-cols-3"><Metric label="Aluguel" value={formatMoney(contract.baseMonthlyRent, contract.currency || 'BRL')} /><Metric label="Vencimento" value={contract.paymentDay ? `Dia ${contract.paymentDay}` : '—'} /><Metric label="Índice" value={contract.rentAdjustmentIndex || '—'} /></div></div>) : <p className="text-[13px] italic text-[#9AA3B0]">Nenhum contrato como inquilino.</p>}</div></DetailCard>;
}
