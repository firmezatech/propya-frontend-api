...
export function FmzTenantTokenPurchasePage() {
  ...
  const proceed = useCallback(() => {
    ...
    router.push(getLocalizedHref('/connected/tokens-to-purchase-pix/confirm'));
  }, [cart, router]);
...
function useRequiredCart() {
  ...
  useEffect(() => {
    ...
    if (!stored) {
      router.replace(getLocalizedHref('/connected/tokens-to-purchase-pix'));
      return;
    }
    setCart(stored);
  }, [router]);
...
export function FmzTenantTokenPurchaseConfirmPage() {
  ...
  const proceed = useCallback(() => {
    ...
    writeTokenPurchaseCart(cart);
    router.push(getLocalizedHref('/connected/tokens-to-purchase-pix/pix'));
  ...
  return (
    <main className={styles.page}>
      <PageBackButton fallback="/connected/tokens-to-purchase-pix" />
      <Stepper active={2} />
      <div className={styles.hero}>
...
export function FmzTenantTokenPurchasePixPage() {
    ...
    const interval = window.setInterval(async () => {
      try {
        ...
        if (statusIsPaid(nextPayment.status)) {
          ...
          setTimeout(() => router.push(getLocalizedHref('/connected/tokens-to-purchase-pix/success')), 1200);
        }
      } catch {
  ...
  return (
    <main className={`${styles.page} ${styles.narrow}`}>
      <PageBackButton fallback="/connected/tokens-to-purchase-pix/confirm" />
      <Stepper active={3} />
