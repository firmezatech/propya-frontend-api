...
import { ArrowLeft, Check, ChevronDown, ChevronRight, FileText, Home, Info, Loader2, Plus, ShieldCheck, Trash2, Users } from 'lucide-react';
  ...
  return (
    <section className="min-h-[calc(100vh-124px)] bg-[#F7F8FA] px-4 py-6 text-[#0D1321] sm:px-6 lg:px-10">
      <div className="mx-auto flex w-full max-w-[1320px] gap-6">
        <aside className="sticky top-[88px] hidden h-[calc(100vh-112px)] w-[224px] shrink-0 rounded-2xl border border-[#E8EAF0] bg-white p-3 shadow-sm xl:block">
          <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-[.1em] text-[#9AA3B0]">Principal</p>
          <SideItem icon={<Home className="h-3.5 w-3.5" />} label="Dashboard" />
          <SideItem icon={<Users className="h-3.5 w-3.5" />} label="Usuários" />
          <SideItem active icon={<ShieldCheck className="h-3.5 w-3.5" />} label="Roles" />
          <SideItem icon={<FileText className="h-3.5 w-3.5" />} label="Imóveis" />
        </aside>
...
function SideItem({ active = false, icon, label }: { active?: boolean; icon: ReactNode; label: string }) {
  return <div className={fmzCn('mb-1 flex items-center gap-2 rounded-lg px-3 py-2 text-[13px] transition', active ? 'bg-[#F0F1F5] font-medium text-[#0D1321]' : 'text-[#5A6478] hover:bg-[#F7F8FA]')}><span className={fmzCn('flex h-7 w-7 items-center justify-center rounded-md border', active ? 'border-[#0D1321] bg-[#0D1321] text-white' : 'border-[#E8EAF0] bg-[#F7F8FA]')}>{icon}</span>{label}</div>;
}
