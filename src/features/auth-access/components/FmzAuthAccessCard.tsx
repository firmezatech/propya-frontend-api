...
import { buildFmzLoginSchema, isValidFmzEmail } from '../domain/fmz-auth-access-validation';
...
type FmzEmailHintState = 'idle' | 'valid' | 'invalid';
...
function useLoginFormState() {
  ...
  const [emailHintState, setEmailHintState] = useState<FmzEmailHintState>('idle');
...
function evaluateFmzEmailHint(rawValue: string): FmzEmailHintState {
  const value = rawValue.trim();
  if (!value) return 'idle';
  return isValidFmzEmail(value) ? 'valid' : 'invalid';
}
      ...
      <div className="shell">
        <aside className="side">
          ...
          <div>
            ...
            <p className="lede">Acompanhe sua participação, veja o aluguel cair e aproveite novas ofertas de imóveis tokenizados ao entrar na sua conta.</p>
          </div>
        </aside>
            ...
            <form ref={formRef} onSubmit={handleFormSubmit} aria-busy={isSubmitting}>
              <div className="stack">
                <div className="f">
                  ...
                  <div className="iw">
                    ...
                    <input
                      ...
                      autoComplete="email"
                      className={fieldErrors.email ? 'invalid' : emailHintState === 'valid' ? 'valid' : undefined}
                      onChange={(event) => setEmailHintState(evaluateFmzEmailHint(event.target.value))}
                    />
                  </div>
                  {fieldErrors.email ? (
                    <FieldError>{fieldErrors.email}</FieldError>
                  ) : emailHintState === 'valid' ? (
                    <span className="hint ok"><Check aria-hidden="true" /> E-mail válido</span>
                  ) : (
                    <span className="hint" />
                  )}
                </div>
                ...
                <div className="f">
                  ...
                  <div className={fieldErrors.password ? 'iw hr invalid' : 'iw hr'}>
                    <Lock className="il" aria-hidden="true" />
                    <input id="password" name="password" type={showPassword ? 'text' : 'password'} placeholder="Digite sua senha" autoComplete="current-password" />
